...
doa_steer_error = 0 *pi/180;    %error in direction
doa_null_error = 0 *pi/180;     %error in direction
...
for m = 1:M-1
  for f = 1:N
    w_c(m+1,f)=exp(-i*(2*pi*w(f)*m*d/c)*sin(doa_steer+doa_steer_error));    % steering vector for this frequency
    w_b(m+1,f)=exp(-i*(2*pi*w(f)*m*d/c)*sin(doa_null+doa_null_error));    % nulling vector for this frequency
  end
end
...
o_f = zeros(1,N);
for f = 1:N
  R = X(:,f)*X(:,f)'; %this should be carried out with past windows
                      %to have a more robust covariance matrix
  
  %fixing steering mismatch in the covariance matrix so that it is inversable
  for m =1:M
    R(m,m) = 1.001*R(m,m) + 0.0000000001;
  end
  inv_R = pinv(R);  %to make sure we inverse it
...
figure(3);
plot(t,s1,t,s2,t,o,'linewidth',5)
axis([0 1 -1 1])
